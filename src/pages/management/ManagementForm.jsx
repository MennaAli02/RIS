...
export default function ManagementForm() {
  ...
  const handlePay = () => {
    const seq = String(Number(id) || Date.now()).slice(-4).padStart(4, '0')
    persist({ state: '2', cash: totalAmountAfterAll - (Number(form.companyShare) || 0), cashReceipt: `INV/2026/${seq}` })
  }

  const handleSaveReport = () => {
    persist({ pdfSaved: true, pdfFilename: `${form.accession || id}.pdf` })
  }

  const handleVerifyReport = () => {
    persist({ reportState: 'verified' })
  }
